...
> 持续完善中...有任何问题请联系作者反馈,转载请注明出处。  
店铺链接: [墨比斯科技](moebius.taobao.com)  
ROS小车购买链接：[->淘宝<-](https://item.taobao.com/item.htm?spm=a230r.1.14.36.26607429V7tZVj&id=620965682048&ns=1&abbucket=5#detail) [->天猫<-](https://detail.tmall.com/item.htm?spm=a230r.1.14.103.6bd07429hZvWJC&id=624172469542&ns=1&abbucket=5)        
作者联系方式: QQ: 1260105099    邮箱: 1260105099@qq.com     
本文链接：[10天零基础玩转ROS小车](http://note.youdao.com/noteshare?id=d257010e4be446276ca2a91230726403&sub=54853CD4728947529F86C6B0A62CB3A4)
...
- [八、SLAM自主导航](#SLAM自主导航)
- [九、机器视觉巡线避障](#机器视觉巡线避障)
- [十、深度机器视觉构建三维点云](#深度机器视觉构建三维点云)

<h2 id="ROS基础">一、ROS基础</h2>
...
<h2 id="环境搭建">二、环境搭建</h2>

树莓派系统安装
[Ubuntu mate 下载链接](https://ubuntu-mate.org/download/)
...
[ROS安装器下载](http://note.youdao.com/noteshare?id=1ef5dfc12838a78a88ddad77eeabaf65&sub=2D88F56A7DA742BE9A7A151467C8D96A)
...
catkin_make
```
<h2 id="ROS分布式通信规范">四、ROS分布式通信规范</h2>
...
1. 包含ROS头文件,主要是一些ROS用到的库和消息类型
```
#include <ros.h>
#include <std_msgs/String.h> 
```
2. 创建一个节点句柄对象,在spinOnce()中处理消息发布、订阅等
```
ros::NodeHandle  nh;
```
3. 示例创建了一个chatter话题对象，引用publish可以发布话题。
```
std_msgs::String str_msg;
ros::Publisher chatter("chatter", &str_msg);
```
4. 初始化ROS节点句柄，将创建的chatter话题注册到通信句柄中
```
nh.initNode();
nh.advertise(chatter);
``` 
5. 通过chatter对象的publish实例将str_msg加入消息队列，通过spinOnce()将消息队列内的消息推送给串口
``` 
chatter.publish( &str_msg );
nh.spinOnce();
``` 
...
2. 定义回调函数，当接收到订阅时调用函数，处理话题发出的消息
```
void messageCb( const std_msgs::Empty& toggle_msg)
{
    digitalWrite(13, HIGH-digitalRead(13));   // blink the led
...
```
3. 创建了一个toggle_led订阅对象，当接收到来自toggle_led话题发布的消息时，调用message函数。
```
ros::Subscriber<std_msgs::Empty> sub("toggle_led", &messageCb );
```
4. 初始化节点，将订阅内容注册到节点句柄中
```
nh.initNode();
nh.subscribe(sub);
```
...
```
在上位机选择对应的.rviz配置文件，就可以看到小车扫描出的地图了
通过按键控制小车运动，就可以将整个屋内的地图扫描出来，接下来将地图文件保存到参数文件内就可以了

<h2 id="SLAM自主导航">八、SLAM自主导航</h2>
**利用激光雷达进行自动导航**
...
<h2 id="机器视觉巡线避障">九、机器视觉巡线避障</h2>

<h2 id="深度机器视觉构建三维点云">十、深度机器视觉构建三维点云</h2>
